...
**real current household Lunch Money state**, not a fixture. Mapping used
the incumbent owner-verified account-id map. The owner can read this packet
and determine the seven acceptance questions without manual reconciliation.
...
unauthorized. Display names remained labels only.

## Current-state gate

Checked on current `main` `bd00afd8badd7c3444f9a005b0214e92f196592c`
(PR #184 merge commit) before the live command. Open pull requests: none.

**Source.** Explicit owner instruction: resume AF-REFRESH-07 — live
closed-loop acceptance and cleanup. Do not invent AF-REFRESH-08.

**Verdict.** STILL BROKEN on that head for this outcome: AF-REFRESH-01
through AF-REFRESH-06 were merged, and PR #184 stopped live acceptance at
the missing trusted account-map gate. This builder environment now holds
the owner-supplied production account-map secret. The gitignored
owner-observed local map file is still absent. The incumbent loader
accepted the secret as `atlas-provider-account-map/v1` before any GET.

**Evidence, before any repository edit.** Incumbent `loadLiveAccountMap`
returned twelve mappings: three household-cash (including the required
chequing-a, chequing-b, and savings identities), five revolving-credit,
one HELOC, one mortgage, and two household-external. A Lunch Money
GET-only credential was present. Display-name reconstruction was not
used.
...
A second live GET of unchanged provider evidence followed. The official
read-only CLI `node scripts/refresh-loop-acceptance.js --live` was run
as a third GET and agreed. Canonical `data.json` remains the 2026-08-19
opening. No `--apply`. Production `/data.json` overlay was not enabled
for this shell (`ATLAS_LIVE_OVERLAY` unset).

Household financial dates use America/Vancouver. The fetch instants
below are UTC; they fall on household date **2026-08-25**.

## 1. What live evidence changed

| Fact | Value |
|---|---|
| Fetched | 2026-08-26T01:04:11.798Z |
| Second GET | 2026-08-26T01:04:12.799Z |
| CLI GET | 2026-08-26T01:05:49.998Z |
...
credit treated as cash. Cash Back pending is live-plan only.
...
rows. No new household policy was inferred. This question does not block
campaign closeout and is not a canonical write.
...
preview. This acceptance did **not** pass `--apply`. Posted `previewId`
cannot authorize pending or an opening. A newer canonical opening remains
an owner-reserved `--approve-opening <openingApprovalId>` approval.
...
Pending overlay is live-plan only. Unmatched cash and unverified owner-fact
rows cannot write. Downstream: Forecast starting cash **would** change by
−$1,031.79 if that preview were later approved. Snapshot and
`representedEvents` would not.
...
**keys** and the posted previewId identical. The later CLI GET produced the
same previewId and spendable cash.
...
| Display-name map reconstruction | refused — owner-verified account-id map was present |
...
remains an owner-reserved `--approve-opening <openingApprovalId>` approval.
`--live` still fails closed before any provider GET when that owner-verified
map is missing.
