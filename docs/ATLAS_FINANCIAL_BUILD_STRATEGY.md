...
| ChatGPT / copilot interface | **ACTIVE** (MCP wrap slice) | `ARCHITECTURE.md` names ChatGPT as a conversational consumer, not an authority. Owner instruction 2026-08-24 reopened this parked row for **read-only assistant access to Atlas**. First slice earned: `GET /assistant/current` via `scripts/assistant-packet.js`, Bearer `ATLAS_ASSISTANT_TOKEN`. Current slice: `POST /assistant/mcp` / `get_atlas_current` wraps that same packet. Not a ChatGPT financial planner. Google Sheet tracks execution the same way. ChatGPT Apps SDK OAuth / in-product installation is the remaining later slice. | Owner asked for the backend/read-only Atlas interface, then the MCP wrap. Never a second financial answer, never the advisory copilot stack, never a Sheet as authority. OAuth app install is a later owner-operated PR. |
