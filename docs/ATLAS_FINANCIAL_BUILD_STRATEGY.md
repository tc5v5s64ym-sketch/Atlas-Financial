...
writes, scheduled refresh, and a newer live substitute opening remain reserved. A production Lunch Money token is authorized for on-demand GET-only observation (2026-08-23), plus the 2026-09-04 one-transaction `category_id` grant recorded in `ARCHITECTURE.md` and the production pass. That grant is not a reusable write class and is not a canonical write credential. Do **not** wait for a second
...
B81 / AF-LIVE-02  trusted canonical refresh
     ← DONE; preview/approve/opening/recovery writers earned;
       ...
       2026-09-04 one-transaction category_id grant in ARCHITECTURE.md;
...
The 2026-09-04 one-transaction `category_id` grant lives in
`ARCHITECTURE.md` and the production pass; it is not a reusable write
class.
...
- **Anything behind a closed gate.** A canonical store remains gated.
  ...
  one-transaction `category_id` grant lives in `ARCHITECTURE.md`; it is
  not a reusable write class.
