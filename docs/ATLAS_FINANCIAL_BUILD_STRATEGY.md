...
- **State** — **opening-cutover writer earned; live production cutover reserved.**
  ...
  constructible. Live `data.json` on `main` remains the 2026-08-16 opening
  until the exact owner-approved 2026-08-19 bytes can be replayed without
  inventing Household evidence. Unattended production writes, a Render
...
| Automated financial-data connectivity / trusted canonical refresh | **ACTIVE** (`B81`) | `ARCHITECTURE.md` connectivity gate. Live **read** observation (`AF-LIVE-01` / `B80` + GET) is **complete**. T4 **passed 2026-08-17**. `AF-LIVE-02` earned posted, pending, and opening-cutover writers in `scripts/canonical-refresh.js`. An approved opening is one coherent canonical + positions + snapshot transition. Live `main` still publishes the 2026-08-16 opening until the exact owner-approved 2026-08-19 bytes can be replayed. | Unattended production writes, a Render token, and a newer live substitute opening remain reserved. Obtain no credential in git. Do not apply the unused $10 or choose a Triangle winner. |
