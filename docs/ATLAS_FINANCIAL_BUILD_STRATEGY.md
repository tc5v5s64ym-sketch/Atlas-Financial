...
writes, scheduled refresh, and a newer live substitute opening remain reserved. A production Lunch Money token is authorized for on-demand GET-only observation (2026-08-23), plus the 2026-09-04 confirmed single-transaction `category_id` exception recorded in `ARCHITECTURE.md` and the production pass. It is not a canonical write credential. Do **not** wait for a second
...
B81 / AF-LIVE-02  trusted canonical refresh
     ← DONE; preview/approve/opening/recovery writers earned;
       ...
       2026-09-04 category_id exception in ARCHITECTURE.md;
...
The 2026-09-04 single-transaction `category_id` exception lives in
`ARCHITECTURE.md` and the production pass.
...
- **Anything behind a closed gate.** A canonical store remains gated.
  ...
  single-transaction `category_id` exception lives in `ARCHITECTURE.md`.
