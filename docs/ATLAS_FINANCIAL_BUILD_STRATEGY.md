...
| ChatGPT / copilot interface | **ACTIVE** (MCP connection slice) | `ARCHITECTURE.md` names ChatGPT as a conversational consumer, not an authority. Owner instruction 2026-08-24 reopened this parked row for **read-only assistant access to Atlas**. First slice (PR #168): `GET /assistant/current` via `scripts/assistant-packet.js`, Bearer `ATLAS_ASSISTANT_TOKEN`. This slice: Streamable HTTP MCP adapter `POST /assistant/mcp` via `scripts/assistant-mcp.js`, one read-only `get_current_state` tool wrapping that packet. Not a ChatGPT financial planner. Google Sheet tracks execution the same way. ChatGPT Apps SDK OAuth is a later adapter if store listing needs it. | Owner asked for the backend/read-only Atlas interface, then the ChatGPT connection. Never a second financial answer, never the advisory copilot stack, never a Sheet as authority. |
