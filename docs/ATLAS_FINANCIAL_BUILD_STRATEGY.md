...
| ChatGPT / copilot interface | **ACTIVE** (first slice) | `ARCHITECTURE.md` names ChatGPT as a conversational consumer, not an authority. Owner instruction 2026-08-24 reopened this parked row for **read-only assistant access to Atlas**: `GET /assistant/current` via `scripts/assistant-packet.js`, Bearer `ATLAS_ASSISTANT_TOKEN`. Not a ChatGPT financial planner. Google Sheet tracks execution the same way. A ChatGPT App / MCP connector is the next slice, not this one. | Owner asked for the backend/read-only Atlas interface. Never a second financial answer, never the advisory copilot stack, never a Sheet as authority. ChatGPT installation/connection is a later PR. |
