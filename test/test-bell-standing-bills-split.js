'use strict';
/* Standing Bell from 2026-10-15 recognizes the proven BILLS two-leg
 * settlement shape. Two same-day chequing-a Bell Mobility debits settle
 * one standing occurrence. Amount is not identity. The September once
 * keeps its own grace and cannot be reused. One payment's transaction
 * ids cannot settle both occurrences. The Travel Visa single-debit rule
 * remains. Synthetic fixtures and independent arithmetic (L-002 / L-006).
 *
...
console.log('\n=== independent amounts, before settlement ===');
{
  ...
  ok(standing && standing.frequency === 'monthly' && standing.day === 15
      ...
      && standing.payingAccount === 'travelvisa' && standing.jointCash === false,
    'canonical standing bell stays $160 monthly from 2026-10-15 on Travel Visa');
...
console.log('\n=== both standing identities are explicit ===');
{
  ...
  ok(rules.length === 2 && visa && bills && once,
    'standing bell has the Travel Visa rule and one chequing-a rule; September stays its own rule');
...
{
  const identity = identityDoc();
  const one = observeWith(identity, OCT_AS_OF, [bellTx(87001, LEG_A)]);
  ok(!standingHit(one) && !septemberHit(one), '1. one Bell debit leg does not settle');
  ...
  ok(!standingHit(pending), 'a pending leg is not posted settlement evidence');
}
