...
| Forecast | `planning.html` | `planning.js` + `forecast.js` | Road Ahead Pay period: each Seaspan period's standalone `stage3.result` (prior-period surplus is not carried in). Road Ahead Month prints two separate Forecast facts: calendar-dated income received in the month (`month.income`, identity `calendar-dated-income`), and available surplus from Seaspan periods whose close falls in that month. A dated requirement cannot use a later close. Those two facts are not required to equal each other. Cumulative walk cash stays internal and is not that funding result. Known future-cost list lives on Plan spend. |
