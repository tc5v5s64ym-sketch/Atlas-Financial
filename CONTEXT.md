...
| Plan spend | `plan-spend.html` | `plan-spend.js` + `forecast.js` | Household-readable cards from `Forecast.majorPlans` and `Forecast.planSpendCards`, in Forecast order. `Forecast.planSpendFundingPaths` links a dated cost to the Road Ahead pay period whose Stage 2 line carries that exact id and date, including a carried cost outside the clipped span, and reprints that period's Stage 2 after-planned-spending result. A Forecast date is a cash-planning date, not a payment due date. A shared overdue `remaining` stays the joint shortfall, not that row's private gap. An unestablished plan-specific saved amount remains unknown. No page financial arithmetic or invented contribution schedule. |
