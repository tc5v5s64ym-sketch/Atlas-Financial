...
| Forecast | `planning.html` | `planning.js` + `forecast.js` | Road Ahead: each month or Seaspan pay period's standalone `stage3.result` (prior-period surplus is not carried in). Cumulative walk cash stays internal and is not that funding result. Known future-cost list lives on Plan spend. |
