...
| Forecast | `planning.html` | `planning.js` + `forecast.js` | Road Ahead Pay period: each Seaspan period's standalone `stage3.result` (prior-period surplus is not carried in). Road Ahead Month: surplus from Seaspan periods whose close falls in that month; a dated requirement cannot use a later close. Cumulative walk cash stays internal and is not that funding result. Known future-cost list lives on Plan spend. |
