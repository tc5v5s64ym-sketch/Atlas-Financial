...
| Plan spend page — known future costs list | Presentation only, in `public/plan-spend.js`, of `Forecast.majorPlans` from the same `Forecast.recommend` call as the Plan page and assistant packet. `Forecast.planSpendCards` retains one card for each `planSpendSummary` group, with its existing sum of member needs and worst published member verdict; this creates no cash event. The page translates internal `ON TRACK` to “FEASIBLE IN CURRENT PLAN” without changing the verdict, keeps `AT RISK` tied to protected uncertainty, and makes `FUNDING GAP` conspicuous. An overdue protected row's `remaining` is the joint as-of shortfall already published for every unsettled protected cost together (`remainingIdentity: joint-protected-overdue-shortfall`); it is not a private allocation to that row. `Forecast.planSpendFundingPaths` links a dated cost to the `baselineTrajectory.payPeriods` entry whose Stage 2 commitment line has that exact id and original date, including a carried unresolved cost whose date sits outside the clipped residual span, and reprints that period's Stage 2 `after-planned-spending` result (`stage2.result`); it publishes no after-planned-spending result for an undated or unrepresented item. The Plan Spend page uses the same `Forecast.baselineTrajectory` inputs Road Ahead uses, including `extraFacilities`. A Forecast commitment `date` is a cash-planning date, not a payment due date. Forecast currently publishes no per-plan saved balance, remaining future-cash-flow need, or serial contribution schedule, so those amounts remain not established and the funding path says unallocated. `paydayAllocation.allocated` is a current-payday allocation, not a saved balance. Cards retain Forecast point/range, dates, timing, confidence and funding order. When the current operating plan is unavailable, the page withholds verdicts; an empty `majorPlans` fails closed. The page reads no `plan.commitments` and performs no financial arithmetic. |
