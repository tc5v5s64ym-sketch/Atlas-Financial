...
6. A new canonical opening still flows through the earned Atlas refresh
   ...
   2026-09-04, for one confirmed single-transaction `category_id` update.
   The exact exception lives on the
   ...
   addendum. The token is not a general Lunch Money write credential and
   not a canonical Atlas write path.
...
as of 2026-09-04, for one confirmed single-transaction `category_id`
update recorded on that production pass. Unattended production writes and
...
- **Trusted canonical refresh** — owner-passed **2026-08-17** (T4).
  ...
  for one confirmed single-transaction `category_id` update. It is not a
  general Lunch Money write credential and not a canonical write path.
...
The 2026-09-04 addendum on that same pass authorizes one confirmed
single-transaction Lunch Money `category_id` update and does not reopen
any other Lunch Money write:
...
2026-09-04, for one confirmed single-transaction `category_id` update.
It is not a canonical write path.
...
confirmed single-transaction `category_id` update after authenticated
household confirmation. Every other Lunch Money interaction remains
GET-only. Unattended production writes remain unauthorized.

**Authorized Lunch Money category correction (not yet implemented).**
Owner decision 2026-09-04: Dale or Amanda may correct one Other Spending
transaction by changing its Lunch Money `category_id`. Atlas may initiate
that write. Lunch Money remains the transaction source of truth.
Forecast remains the sole planner and calculation authority. After Lunch
Money confirms success, Atlas returns to the incumbent GET observation
and Forecast recomputes from observed state. Atlas must not keep a
competing local transaction ledger and must not mutate published
financial state before that confirmation. The application boundary for a
later implementation PR is recorded on the production-pass addendum.
This paragraph authorizes that later PR; it does not add the write seam.
...
a bank action: it authorizes only a confirmed `category_id` update on one
exact Lunch Money transaction, on the evidence feed, as recorded on the
production-pass addendum. It does not authorize transfers, payments,
...
`/data.json` observation. Atlas usage remains GET-only except the
2026-09-04 confirmed single-transaction `category_id` update recorded on
that pass, even though Lunch Money personal tokens are not
provider-scoped read-only. Never for anything on the absolute list.
...
- **production** — the deployment platform's environment secrets, which is Render
  ...
  and, as of 2026-09-04, for the confirmed single-transaction `category_id`
  update on that production pass. It remains server-side and is never
