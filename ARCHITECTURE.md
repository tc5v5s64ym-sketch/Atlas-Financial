...
`public/plan.js` holds the wording only. Diagnostics stay folded.
...
| Plan payday operating sheet — the default Plan first screen | Presentation only, in `public/plan.js`, of one `Forecast.recommend` result (`defaultView`). Current Balance is `paydayAllocation.available`, never overdraft and never `chequingAvailability`, labelled Current Balance with the live overlay as-of (time from overlay `fetchedAt` when present). Bills this pay period are `currentPeriodAction.thisPeriodGlance`: from this payday date through this period, paid stay listed as PAID, still due / pending copy Forecast settlement without printing settlement code words. Previous-cycle once stubs and mid-period inflows before this payday are omitted. Running leftover after bills, household budget, extra debt, and big-purchase set-aside is `paydayAllocation.runningLeftover` / `recommend.defaultView` — the page does not subtract. Household budget prints existing `plan.budget` owner-target lines (groceries, fuel, dog food, eating out); it does not invent Dale/Amanda rows. First card is `debtPriority` / `extraDebt` extra this payday (including $0). Other revolving cards list balance, rate, minimum; HELOC and mortgage are omitted. Big purchases copy `majorPlans` date/cost with saved $0 unless Forecast already set aside this payday. Glance movements print money in as + and money out as − from Forecast `movement` fields without flipping leftover math. Weekly cap, allocation sheet, and 13-week diagnostics stay folded. Distinct from `Forecast.nextMove` / `plan.actions`. Not a second planner, payday engine, goals engine, or freshness calculator |
