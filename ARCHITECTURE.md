...
is the payday operating sheet: one spendable cash figure
(`Forecast.paydayAllocation.available`, not overdraft), required bills with
unverified labelled unverified, `Forecast.recommend` weekly cap or infeasible,
extra debt only from `paydayAllocation` true surplus, `Forecast.majorPlans`
set-aside versus later ON TRACK, and one next-move instruction copied from
those same Forecast fields. Remainder after allocation is not the next move.
