...
| Hypothetical extra payment against one eligible debt | `Forecast.hypotheticalExtraPayment`, composing `Forecast.simulate` and `Forecast.projectDebts`. The caller supplies a finite positive whole-cent amount, one stable eligible debt id (revolving unsecured or HELOC), the plan-opening as-of, and `nature: hypothetical`. The extra is one dated `kind: extra` event on that as-of, not monthly `extraDebtMonthly` and not `Forecast.counterfactuals`. Owner `plan.nextDollar` is not substituted for the caller target; leftover does not cascade to the priority chain. `defaults.targetBuffer` is not a payment, spendable-cash, or breathing-room answer. Deltas come only from those Forecast walks. Consequences Forecast cannot establish are omitted rather than zeroed. The amount is not spendable cash; available credit is not cash. No recommendation and no action permission. Stateless and read-only. Pages, Talk, and Gemini do not recalculate. Talk NL extract of amount/target is a later slice |
