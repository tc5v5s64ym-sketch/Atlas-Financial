...
| Cash projection over the window | `Forecast.simulate`. `opts.horizonDays` walks the master; `opts.viewDays` slices the same walk for display; `opts.viewStart` may begin that slice after as-of so a custom range is a real offset, not an as-of prefix of the same length. A sliced week recomputes its in-range income, bills, obligations, variable, reserved and other aggregates from the clipped events and days; it does not keep the original week's full totals. Default remains `plan.windowDays` so a caller that has not asked for the master still gets the view. Undated `plan.budget.categories[].currentMonthly` (when it wins over an owner target) is reserved as daily cash on this walk, using the calendar month `365.25 / 12`, so a card-paid current-regime service without a trustworthy day exerts gravity without a fabricated due date. A dated card-paid bill is reserved on its planning day on the same `week.reserved` / `totals.reserved` ledger, not as `week.bills` and not mixed into the weekly-cap variable/Budget column. Neither form is a second event engine. Spendable opening is `startingCashAmount`: household chequing-only (`HOUSEHOLD_CHEQUING_IDS`, the same identities as `Forecast.postedHouseholdChequingCash`) when those rows exist on the breakdown; designated `savings` / EMERGENCY SAVING stays on the breakdown as reserve evidence and is not spent on the walk (owner 2026-09-18, Case K). `additionalCashRequired` is `max(0, 0 − min.balance)` on that chequing-only carry-forward walk — the same zero-floor identity as `shortfall`. Funding floor is $0, not `targetBuffer`, and not a new emergency buffer. Period month/pay-period `stage*.result` deficits must never be summed to derive this requirement |
