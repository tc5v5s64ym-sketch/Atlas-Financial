...
5. Today's live plan may overlay current posted balances and pending
   ...
   dated opening. Freshness-qualified posted cash is still attached as
...
| Plan payday operating sheet — the default Plan first screen | Presentation only, in `public/plan.js`, of one `Forecast.recommend` result. The default print is `defaultView`. A picker may print `nextPeriodView` or one `weekViews` row instead; the page does not subtract leftover or invent a week. Current Balance on the default view is `paydayAllocation.available`, never overdraft and never `chequingAvailability`, labelled Current Balance with the live overlay as-of (time from overlay `fetchedAt` when present). Next-period Current Balance is the already-run `recommend.sim` start-of-day cash on the next Forecast payday plus that day's income. A week view's Current Balance is that simulated week's opening. Default-view bills are `recommend.defaultView.billSections`: two calendar halves of the as-of month (1st–15th and 16th–last day), from `expandEvents` dates, each row carrying name, amount, due date, and planned paying account. Those halves are bill-planning buckets, not income-event boundaries and not leftover math. `thisPayday` remains payday-date events only. Undated Bell (`needsDate`) is printed once as still needing a date and does not invent a day. Lookahead and week bills stay that span's `expandEvents` with Forecast settlement, paid stay listed as PAID, still due / pending copy settlement without printing settlement code words. Running leftover after bills, household budget, extra debt, and big-purchase set-aside is `paydayAllocation.runningLeftover` / `recommend.defaultView` on the default view. Next-period and week leftover use the same leftover identity on that span's unpaid joint-cash bills and owner-target hold (7-day scale for a week, that period's days for the next payday); extra and big-purchase set-aside stay $0 on those spans because they are this-payday surplus decisions. Household budget prints existing `plan.budget` owner-target lines (groceries, fuel, dog food, eating out); it does not invent Dale/Amanda rows. After the ten-block, the selected view (`defaultView` / `nextPeriodView` / one `weekViews` row) also prints `budgetDigest`: each existing owner-target category, `spent $X of $Y`, where spent is classified live overlay actuals in that span and Y is the owner-target monthly scaled to that span's days (the same hold scale next-period/week household budget already uses — not leftover remaining-cap, not the unscaled monthly target). Incomplete actuals or a spending-history as-of behind the view as-of are kitchen-counter words; remainingClaim codes stay off the page. The page does not subtract leftover. First card is `debtPriority` / `extraDebt` extra this payday on the default view (including $0). Other revolving cards list balance, rate, minimum; HELOC and mortgage are omitted. Big purchases copy `majorPlans` date/cost with saved $0 unless Forecast already set aside this payday; lookahead and week lists are that span's dated purchases only. Glance movements print money in as + and money out as − from Forecast `movement` fields without flipping leftover math. Weekly cap, allocation sheet, and 13-week diagnostics stay folded. Distinct from `Forecast.nextMove` / `plan.actions`. Not a second planner, payday engine, goals engine, or freshness calculator |
