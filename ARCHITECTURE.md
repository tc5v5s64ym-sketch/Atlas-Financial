...
5. Today's live plan may overlay current posted balances and pending
   ...
   action, next cash-out, Amanda salary has-to-land-by guidance, and
   the current Consumer debt hero tile are withheld. Freshness-qualified posted cash is still attached as
...
6. A new canonical opening still flows through the earned Atlas refresh
   ...
   authorised **only** for on-demand GET-only observation as of
   2026-08-23.
...
- **Trusted canonical refresh** — owner-passed **2026-08-17** (T4).
  ...
  **only** for the on-demand read-only overlay, not for canonical writes.
...
is authorized for on-demand GET-only observation (2026-08-23), not for
canonical writes.
...
observation. It does not authorize unattended production writes.
...
permits that same GET-only token in Render for on-demand authenticated
`/data.json` observation. Atlas usage remains GET-only
even though Lunch Money personal tokens are not provider-scoped read-only.
Never for anything on the absolute list. Unattended production writes remain
not authorised.
...
- **production** — the deployment platform's environment secrets, which is Render
  ...
  GET-only token in Render is authorised for on-demand live observation
  (2026-08-23). The provider-account map reaches production as
