...
statutory calculator. The Road Ahead walk and further-future Budget
timeline rows both consume that source. The estimate is applied only on
further-future rows, for Dale payroll dates from 2027-01-01, and those
rows are marked estimated (`incomeRegime` `2027-estimated`). It is not
applied to the current row, Budget's next row, `pastPeriodViews`, or
`nextPeriodView`. The 2026 modelled net is not carried into 2027. A
2027 row's Dale income can shift when that period becomes Budget's
current or next row, because those rows stay on the incumbent printer
