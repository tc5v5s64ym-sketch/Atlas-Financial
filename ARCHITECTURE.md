...
canonical write and not a new cash store. Forecast then adds posted
chequing-a movements dated that payday which are not a plausible
unrecognised Dale deposit (Seaspan merchant text, or an amount within 1%
of the planned payroll), using `Forecast.postedAccountMovements`, and then
adds the planned payroll. Two such inflows, or a payday movement packet
that is present but incomplete, fail closed. The refreshed BILLS row is
never that base and is never increased by the planned payroll. The assumed amount is labelled
