...
6. A new canonical opening still flows through the earned Atlas refresh
   ...
   2026-09-04, for the one owner-granted `category_id` correction on one
   exact owner-identified transaction only. The exact exception lives on
   the
   ...
   addendum. That grant is not a reusable write class and does not make
   the token a standing write credential. The token is not a general
   Lunch Money write credential and not a canonical Atlas write path.
...
as of 2026-09-04, for the one owner-granted `category_id` correction on
one exact owner-identified transaction only, recorded on that production
pass. That grant is not a reusable write class. Unattended production writes and
...
- **Trusted canonical refresh** — owner-passed **2026-08-17** (T4).
  ...
  for the one owner-granted `category_id` correction on one exact
  owner-identified transaction only. That grant is not a reusable write
  class and does not make the token a standing write credential. It is
  not a general Lunch Money write credential and not a canonical write
  path.
...
The 2026-09-04 addendum on that same pass authorizes one specific
Lunch Money `category_id` correction on one exact owner-identified
transaction only and does not reopen any other Lunch Money write:
...
2026-09-04, for the one owner-granted `category_id` correction on one
exact owner-identified transaction only. That grant is not a reusable
write class. It is not a canonical write path.
...
observation. The 2026-09-04 addendum on that pass authorizes one
specific `category_id` correction on one exact owner-identified
transaction only. Every other Lunch Money interaction remains GET-only.
A reusable category-edit capability remains unauthorized unless the
owner separately grants it. Unattended production writes remain
unauthorized.

**One authorized Lunch Money category correction (not a write class).**
Owner decision 2026-09-04: Atlas may perform the one `category_id`
correction the owner already granted on the one owner-identified Lunch
Money transaction. That grant is exhausted by that one write. It is not
Household Budget / Other Spending editing for arbitrary transactions,
not “one exact transaction at a time,” and not a standing
production-token write class. Lunch Money remains the transaction
source of truth. Forecast remains the sole planner and calculation
authority. After Lunch Money confirms that one write, Atlas returns to
the incumbent GET observation and Forecast recomputes from observed
state. Atlas must not keep a competing local transaction ledger and
must not mutate published financial state before that confirmation.
This paragraph records that one grant; it does not add a write seam.
...
other financial institution. The 2026-09-04 Lunch Money exception is not
a bank action: it authorizes only the one owner-granted `category_id`
correction on one exact owner-identified transaction, on the evidence
feed, as recorded on the production-pass addendum. It is not a reusable
write class. It does not authorize transfers, payments, amount edits, or
any other provider write.
...
`/data.json` observation. Atlas usage remains GET-only except the one
2026-09-04 owner-granted `category_id` correction on one exact
owner-identified transaction recorded on that pass, even though Lunch
Money personal tokens are not provider-scoped read-only. That grant is
not a standing write class. Never for anything on the absolute list.
Unattended production writes remain not authorised. The token does not
become a general-purpose Atlas write credential.
...
- **production** — the deployment platform's environment secrets, which is Render
  ...
  and, as of 2026-09-04, for the one owner-granted `category_id`
  correction on one exact owner-identified transaction only. That grant
  is not a standing write class. It remains server-side and is never
  published to the browser. The provider-account map reaches production as
