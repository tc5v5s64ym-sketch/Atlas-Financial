...
household-facing status stays planned. It is not labelled PAID. A
represented key dated on or before that financial date, and every past
or current row, keeps the incumbent status. Because the next timeline
row is the same object as `calendarPeriods` id `next-pay-period`, that
status label can change on Budget's Next Pay Period while the amount and
Balance After Deductions do not. The owner-approved $400 Other Spend
