...
| Plan spend page — known future costs list | Presentation only, in `public/plan-spend.js`, of `majorPlans`, `paydayAllocation` (`futureCosts` / `optional` / `unresolved`) and `knowledge` from the same `Forecast.recommend` call the Plan page, Forecast page composer, and the assistant packet make. Isolated list/detail surface of Forecast-published planned spending from `majorPlans` (plan.commitments plus yearly card-paid `plan.bills`, each once; Square One is the dated yearly card-paid bill, not a second commitment). Road Ahead on Forecast remains the funding story. Cards follow `majorPlans` order. A `plan.groups` row with `planSpendSummary: true` is one display card at its first published member, via `Forecast.planSpendCards`: the card's schedule remaining is the sum of those members' published `majorPlans.need` values and nothing else. That sum is not a cash event, does not merge dated instalments, and is not an input to `simulate` or `recommend`. Other rows stay one card each. Verdict, remaining, range or point, `when`, `tripWindow` when Forecast published one, flexibility, confidence, `deferred`, and any `allocated` / `projectedByDeadline` are copied; secondary facts may sit in disclosure. The protected-floor aggregate remains `knowledge.encumbered`, printed as Forecast's protected floor with ranges at their low end. No saved balance is invented. When the current operating plan is unavailable the page withholds verdicts. An empty `majorPlans` list fail-closes empty. Does not read `plan.commitments`. Not a second planner, sequence, total, or cash walk |
