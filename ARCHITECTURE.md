...
| Payday allocation — what current household cash must do this payday | `Forecast.paydayAllocation`. One waterfall on current spendable cash plus same-day income: required joint-cash obligations (including overdue protected floors and commitments due this period), essential household costs from incumbent budget classifications (a cash hold, not a second spend permission — `Forecast.recommend` remains the weekly spending authority), dynamic liquidity from the near-term cash path until subsequent income (the existing model buffer is the feasibility floor, not a fixed payday cushion and not Q20), required future-cost funding pressure (the minimum now so the same cash walk stays jointly feasible through later household deficits and dated protected commitments: a later household deficit consumes earlier surplus before a commitment can claim that dollar; an uncovered later deficit that exceeds earlier surplus remains current path pressure and is not clipped away; not remaining÷paycheques; not independent non-negative payday buckets), extra debt via the incumbent unsecured-then-HELOC absorbable cascade, then owner-marked optional residual. Required or bounded-flex amounts with no exact date stay unresolved: no fabricated date, no mandatory contribution, no silent demotion to optional residual, and no release of leftover cash to extra debt or optional residual unless owner-authorized semantics already mark the item optional or otherwise releasable. Pages render every material allocation line. Credit and unapproved borrowing are not cash. Allocated dollars plus unallocated remainder equal available resources. `Forecast.recommend` attaches this same result; the page does not compute it |
