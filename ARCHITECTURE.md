...
| Baseline cash-and-debt trajectory over the knowledge horizon | `Forecast.baselineTrajectory`, composing incumbent `Forecast.knowledgeHorizon`, `Forecast.budgetBreakdown` planned weeklyVariable (owner-target / current-regime Household Budget planned remainder — not historical actuals and not `Forecast.recommend`'s feasible weekly cap), `Forecast.simulate`, and coupled `Forecast.projectDebts`. The monthly series covers every calendar month that intersects the incumbent knowledge horizon. It does not extend `knowledgeHorizon`, change published caps, or year-stop global `expandEvents` / `simulate` / `recommend`. Named dated income-regime split (trajectory-local, `isDalePayrollStream` and `Forecast.daleEstimatedPayrollDeposits`): current modelled Dale net through 2026-12-31; Dale payroll/bonus from 2027-01-01 through 2027-12-31 is an evidence-derived ESTIMATED regime when Seaspan salary evidence is present (deposit-year CPP/EI reset; planning estimate — not verified future pay), not $0 and not expandEvents-carried 2026 post-CPP/EI-max net. After 2027-12-31 that path fails closed until a later regime is authorized and independently proved — not the 2027 raise, bonus rate, or last-published statutory tables carried forward. Streams without Seaspan evidence stay fail-closed unavailable. Dale/Seaspan-owned bonus uses the same ownership signals as Dale payroll; Amanda/other bonus is not treated as Dale. Owner-authorized planning assumptions (future raise factor, bonus rate) are labelled ESTIMATED inside that trajectory-local engine; they are not a second owner-policy store and never promote to verified. `incomeRegimesImplemented` is true only when that Seaspan-evidenced 2027 Dale net exists. Only an income occurrence actually settled into the relevant opening may replace its estimate; a future represented date cannot suppress an unsettled estimate. Cash after 2027-01-01 is estimated on that walk through 2027-12-31, or unavailable while withheld Dale income would otherwise enter it. Read-only. `public/planning.js` reprints the monthly series (status, amounts, notes) and does not recalculate. `scripts/assistant-packet.js` projects the same reprint as `planning.trajectory` on `GET /assistant/current`, MCP `get_atlas_current`, and session `GET /talk/context` (same `Forecast.baselineTrajectory` call; not gated on `liveOverlay.operatingPlan`). Talk Ask NL trajectory templates remain deferred; Talk does not reprint trajectory in household answers. Not a second planner, not ranking, recommendation, or affordability. |
