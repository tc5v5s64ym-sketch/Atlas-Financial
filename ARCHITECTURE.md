...
| Cash projection over the window | `Forecast.simulate`. `opts.horizonDays` walks the master; `opts.viewDays` slices the same walk for display; `opts.viewStart` may begin that slice after as-of so a custom range is a real offset, not an as-of prefix of the same length. A sliced week recomputes its in-range income, bills, obligations, variable and other aggregates from the clipped events and days; it does not keep the original week's full totals. Default remains `plan.windowDays` so a caller that has not asked for the master still gets the view. Undated `plan.budget.categories[].currentMonthly` (when it wins over an owner target) is reserved as daily cash on this walk, using the calendar month `365.25 / 12`, so a card-paid current-regime service exerts gravity without a due date; it shares the variable ledger column and is not a second event engine |
