...
| ChatGPT | conversational query, explanation and scenario interface. It consumes `GET /assistant/current` and the Streamable HTTP MCP adapter at `POST /assistant/mcp` (this file's read-only assistant surface). It is not a planner. |
...
| Read-only assistant access to Atlas | `scripts/assistant-packet.js` projects incumbent Forecast, live-overlay, and `public/periods.json` results onto `GET /assistant/current`. `scripts/assistant-mcp.js` is the Streamable HTTP adapter at `POST /assistant/mcp`: one read-only tool (`get_current_state`) that returns that same packet. Authenticated by dedicated `ATLAS_ASSISTANT_TOKEN` as `Authorization: Bearer`. Not a planner, not a second Forecast, not a copy of household facts, and not a transaction store. Unavailable answers stay unavailable. Individual projected obligations keep the incumbent Forecast event confidence (`confirmed` / `estimated` / `planned`); grouped obligations (`nextPaymentOut`, near-boundary totals) take the weakest constituent confidence and never promote estimated to confirmed. Never writes. ChatGPT Apps SDK OAuth remains a later adapter and is not this authority. |
