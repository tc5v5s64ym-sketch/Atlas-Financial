...
| Current funding-option available / revolving headroom used to cover an opening gap | `Forecast.resolveFundingSources`, from current `plan.startingCash` (`cash` locators), `Forecast.utilisation` (`debtId` locators and residual unclaimed revolving rows), and extra facilities. Stored `plan.funding.options[].available` is not current-state. `Forecast.recommend` consumes that view. `public/plan.js` holds the wording only |
