...
estimated-regime end (`plan.payrollPlanningAssumptions` via
`Forecast.daleEstimatedPayrollDeposits`). Owner Dale, 2026-09-25
7:43 PM PT: "Use the 2027 estimated payroll for 2027 pay periods, marked
as estimated." That regime is applied only on further-future rows, for
Dale payroll dates from 2027-01-01, and those rows are marked estimated
(`incomeRegime` `2027-estimated`). It is not applied to the current row,
Budget's next row, `pastPeriodViews`, or `nextPeriodView`. The 2026
modelled net is not carried into 2027. A 2027 row's Dale income can
shift when that period becomes Budget's current or next row, because
those rows stay on the incumbent printer rather than the estimate.
...
term. `liveCurrentBalance` stays null off the current row. An occurrence
already represented inside the cutover opening can still print PAID on a
later row; that occurrence is not deleted, because dropping it would
double-count cash already in the opening. The owner-approved $400 Other
Spend estimate for future periods is deferred: this publication adds no
