...
| Budget — owner targets against actuals, including the grocery and fuel monthly figures the Plan page prints | `Forecast.budgetBreakdown`, with classification and targets in `data.json` `plan.budget`. The `cap` block owns the food-and-fuel weekly pair and the monthly grocery/fuel amounts (owner target if present, else historical). `public/plan.js` holds the wording only |
