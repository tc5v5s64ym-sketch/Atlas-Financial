...
5. Today's live plan may overlay current posted balances and pending
   ...
   dated opening. Same-day scheduled cash events need
...
| ChatGPT | conversational query, explanation and scenario interface. It consumes `GET /assistant/current` (this file's read-only assistant surface). It is not a planner. |
...
- **production** — the deployment platform's environment secrets, which is Render
  ...
  from `SITE_PASSWORD` and from the browser session. Unattended canonical writes
