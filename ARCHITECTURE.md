...
| Plan payday operating sheet — the six glance answers on the live Plan first screen | Presentation only, in `public/plan.js`, of one `Forecast.recommend` result. Cash is `paydayAllocation.available` labelled with as-of / live-overlay provenance, never overdraft and never `chequingAvailability`. Bills are `paydayAllocation.obligations` with `unverified` labelled unverified. Weekly cap is `recommend.weekly`, or infeasible with the failing constraint — never a fake `$0/week` yes. Extra debt is `paydayAllocation.extraDebt` only when allocated surplus exists. Big purchases are `majorPlans` plus `paydayAllocation.futureCosts` this-payday set-asides versus later ON TRACK. The next-move instruction is copied from `currentPeriodAction` / those same fields; remainder after allocation is not the instruction. Distinct from `Forecast.nextMove` / `plan.actions`. Not a second planner, payday engine, goals engine, or freshness calculator |
