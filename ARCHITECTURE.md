...
| Read-only assistant access to Atlas | `scripts/assistant-packet.js` projects incumbent Forecast, live-overlay, and `public/periods.json` results onto `GET /assistant/current`. That incumbent direct surface is authenticated by dedicated `ATLAS_ASSISTANT_TOKEN` as `Authorization: Bearer`. `scripts/assistant-mcp.js` exposes the same projection as exactly one read-only MCP tool (`get_atlas_current` on `POST /assistant/mcp`); `scripts/assistant-oauth.js` is its OAuth protected-resource boundary. MCP accepts only issuer-signed access tokens for the exact resource and `atlas.current.read` scope; the browser cookie and static assistant token remain separate and do not work there. Login, consent, authorization-code + PKCE, client registration, token issuance, and refresh remain with the configured external authorization server. MCP is transport only and OAuth is authentication/authorization only. Neither is a planner, a second Forecast, a copy of household facts, or a transaction store. Unavailable answers stay unavailable. Individual projected obligations keep the incumbent Forecast event confidence (`confirmed` / `estimated` / `planned`); grouped obligations (`nextPaymentOut`, near-boundary totals) take the weakest constituent confidence and never promote estimated to confirmed. Never writes. |
...
server: it may validate an incoming short-lived access token for the exact MCP
resource and `atlas.current.read` scope, then discards it. It does not issue,
refresh, persist, log, or return that token and does not add an Atlas user store.
...
session. It exists only in the `Authorization` header long enough to verify its
issuer signature/JWKS, audience/resource, expiry/not-before, and read scope. It
is never stored, logged, returned, or forwarded into the assistant packet.
