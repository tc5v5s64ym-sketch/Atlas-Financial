...
5. Today's live plan may overlay current posted balances and pending
   ...
   bills / Predicted Ending Balance as today's operating waterfall) are withheld. The Plan
...
from `Forecast.postedHouseholdChequingCash` remains posted household chequing
cash (Chequing A / BILLS ACCOUNT plus Chequing B / WEEKLY SPENDING, including
a negative Chequing B register) and is the walk / paydayAllocation figure.
The Budget page prints that same Forecast Current Balance. Unused
overdraft capacity still never enters. Forecast walk and paydayAllocation spendable
opening (`startingCashAmount`) uses that same chequing-only pool. Designated
...
TENNIS INCOME, not overdraft, and not same-day arriving income, and is printed outside the payday
...
assigned against the frozen payday snapshot, including subsequently
...
otherwise. Live-cash not-relied-upon settlement does not drop that
income term. Those four terms are published on
`predictedEndingBalanceTerms` and must arithmetically close; there is
no balancing plug. The
payday-boundary opening is posted Chequing A (BILLS ACCOUNT) plus
Chequing B (WEEKLY SPENDING), including a negative Weekly carry; it is
not chequing-a posted-balance-observed-on-household-date, not a
transfer-history total, and not today's live Current Balance. It fails closed
...
Spending actual. Extra on the highest-rate revolving card, other
revolving cards, and big purchases follow that Predicted Ending
Balance. `paydayAllocation.runningLeftover` remains the current-cash
