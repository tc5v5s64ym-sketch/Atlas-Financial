...
- **Every dated item declares where it would otherwise sit.** A bill or
  ...
  A dated card-paid bill (Bell Mobility $121 on the 15th) is reserved on
...
The Budget page's printed Current Balance is display composition only of those
same Forecast-owned rows (live `observedCash` when that packet is complete,
otherwise the opening breakdown): `chequing-a + max(0, chequing-b)`. A
Chequing B overdraft is ignored in that print and is not subtracted. Unused
