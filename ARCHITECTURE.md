...
**Plaid read-only balance — foundation, not completion (owner instruction
2026-09-11 via the Atlas Coordinator).** Atlas may make one read-only Plaid
request, `POST /accounts/balance/get`, and normalize the returned account
balances into the incumbent provider-observation boundary
(`scripts/plaid-balance.js` → `scripts/provider-observe.js --provider plaid`
→ `scripts/reconcile.js`). That is the whole grant. It does **not**
authorize Plaid Transactions, Transactions Refresh, webhooks, Plaid Link
onboarding inside Atlas, a second planner, a canonical store, a parallel
current-balance authority, or any change to the live overlay: Lunch Money
remains the incumbent live provider and `scripts/live-plan.js` does not
consume Plaid. Production is unchanged unless Plaid configuration is
explicitly present. Plaid is a data provider the owner authorizes at Plaid;
its application credentials and Item `access_token` are configured secrets
under **The secret boundary**, not institution login credentials. A mocked
Plaid success is not live TD connectivity proof. The named next step is
owner-authenticated Plaid Link onboarding plus one live TD acceptance (TD
itself vs Plaid balance vs Lunch Money); it is a separate owner-gated
outcome, not implied by this seam.
...
The owner instruction of 2026-09-11 separately permits the Plaid
application credential pair and one Item `access_token` —
`PLAID_CLIENT_ID`, `PLAID_SECRET`, `PLAID_ACCESS_TOKEN`, with `PLAID_ENV`
naming `sandbox` or `production` — as configured secrets for exactly one
read-only request, `POST /accounts/balance/get`. They are Plaid-issued
service credentials, not a bank username, password, PIN or code, and they
never authenticate Atlas *as the household*. They live only in the owner's
shell or the deployment platform's environment secrets, travel only in the
server-side request body, and are never placed in a URL, log, error
message, report, fixture, `data.json`, pull request or the browser. The
Plaid account map reaches production as `ATLAS_PLAID_ACCOUNT_MAP_JSON`, the
same `atlas-provider-account-map/v1` schema with `provider: plaid`. Nothing
here authorizes a Plaid write, Plaid Link inside Atlas, or a rotated /
