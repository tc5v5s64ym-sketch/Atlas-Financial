...
| Payday allocation — what current household cash must do this payday | `Forecast.paydayAllocation`. One waterfall on current spendable cash plus same-day income: required joint-cash obligations (including overdue protected floors and commitments due this period), essential household costs from incumbent budget classifications, dynamic liquidity from the near-term cash path until subsequent income (the existing model buffer is the feasibility floor, not a fixed payday cushion and not Q20), required future-cost funding pressure (the minimum now so dated protected commitments stay jointly feasible given modeled future income; not remaining÷paycheques; undated amounts get no fabricated date or mandatory contribution), extra debt via the incumbent unsecured-then-HELOC absorbable cascade, then optional residual. Pages render the result. Credit and unapproved borrowing are not cash. Allocated dollars plus unallocated remainder equal available resources. `Forecast.recommend` attaches this same result; the page does not compute it |
