...
| ChatGPT | conversational query, explanation and scenario interface. It consumes `GET /assistant/current` and the read-only MCP wrapper `POST /assistant/mcp` (this file's assistant surface). It is not a planner. |
...
| Read-only assistant access to Atlas | `scripts/assistant-packet.js` projects incumbent Forecast, live-overlay, and `public/periods.json` results onto `GET /assistant/current`. Authenticated by dedicated `ATLAS_ASSISTANT_TOKEN` as `Authorization: Bearer`. Not a planner, not a second Forecast, not a copy of household facts, and not a transaction store. Unavailable answers stay unavailable. Individual projected obligations keep the incumbent Forecast event confidence (`confirmed` / `estimated` / `planned`); grouped obligations (`nextPaymentOut`, near-boundary totals) take the weakest constituent confidence and never promote estimated to confirmed. Never writes. `scripts/assistant-mcp.js` wraps that same GET as one read-only MCP tool (`get_atlas_current` on `POST /assistant/mcp`) with the same Bearer gate; it is not a second packet builder and not a planner. ChatGPT Apps SDK OAuth remains a later adapter and is not this authority. |
