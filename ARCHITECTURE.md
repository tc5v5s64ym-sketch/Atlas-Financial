...
| Plan payday operating sheet — the six glance answers on the live Plan first screen | Presentation only, in `public/plan.js`, of one `Forecast.recommend` result. Cash is `paydayAllocation.available` labelled Current cash flow, never overdraft and never `chequingAvailability`. Still due is `currentPeriodAction.thisPaydayDue` (unpaid remaining from this payday's leftover cash: overdue recurring obligations such as Travel Visa min, and bills due on or after this payday such as TD fees). Already paid is `currentPeriodAction.thisPaydayPaid` (represented income and paid bills dated on this payday only — the cheque date — not the whole current-period represented set; a card min due earlier can appear here when an identified extra payment posted on payday). Glance movement lines print money in as + and money out as − from Forecast `movement` fields without flipping leftover or allocation math. Previous-cycle once stubs and mid-period inflows are omitted from the default glance; posting-unknown wording is not printed there. Weekly cap is `recommend.weekly`, or infeasible with the failing constraint — never a fake `$0/week` yes. Extra on the cards is `paydayAllocation.extraDebt` only when allocated surplus exists. Big purchases stay behind disclosure. The next-move instruction is copied from `currentPeriodAction` / those same fields; remainder after allocation is not the instruction. Distinct from `Forecast.nextMove` / `plan.actions`. Not a second planner, payday engine, goals engine, or freshness calculator |
