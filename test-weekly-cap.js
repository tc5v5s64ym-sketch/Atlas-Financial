...
(async () => {
  ...
  ok(/\$217/.test(capSplit), 'and $217/wk of discretionary room');
  ok(/Groceries \$414, fuel \$299/.test(capSplit), 'with groceries and fuel split per week');
  ok(/≈ \$4,935 a month/.test(capSplit), 'and the cap said in months');
  const roomNote = h => (/own budget for those comes to [^<]*/.exec(h) || ['(not found)'])[0];
  ok(/so the plan is \$637\/wk short of it and something has to give\./.test(capSplit),
    'and the household-budget sentence reads exactly as it does today', roomNote(capSplit));
  ok(/\$4,935 \/ month/.test(budgetOut), 'the ledger says the same monthly cap');
  ok(/\$217 \/ week/.test(budgetOut) && !/nothing left/.test(budgetOut),
  ...
  ok(/\$7,704\/month against a cap of \$4,935\/month/.test(catsNote)
    && /\$2,769\/month has to come off/.test(catsNote),
