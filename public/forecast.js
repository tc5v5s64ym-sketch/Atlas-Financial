...
(function (root) {
  ...
  function isAmazonPrimeMerchant(tx) {
    if (tx && tx.subscriptionBill === true) return true;
  ...
  // Pending and posted copies of one purchase. Overlay already collapses
  // the same providerTransactionId; Lunch Money often issues a new id
  // when pending settles, so Forecast also keys date + account + amount
  // + merchant and counts the posted / confirmed-settled row once.
  function spendIdentityKey(tx) {
    if (!tx || !tx.date) return null;
    const account = tx.atlasAccountId || tx.account || tx.accountId;
    if (account == null || account === '') return null;
    const amt = Number(tx.amount);
    if (!isFinite(amt) || amt === 0) return null;
    const merchant = normalizeMerchantKey(txMerchantExact(tx));
    if (!merchant) return null;
    return String(tx.date) + '|' + String(account) + '|' + roundCent(amt).toFixed(2) + '|' + merchant;
  }

  function skipPendingPostedDuplicate(tx, packet) {
    if (!tx || transactionPendingState(tx) !== 'pending') return false;
    const key = spendIdentityKey(tx);
    if (!key) return false;
    const txs = packet && Array.isArray(packet.transactions) ? packet.transactions : [];
    return txs.some(other => {
      if (!other || other === tx) return false;
      if (transactionPendingState(other) === 'pending') return false;
      return spendIdentityKey(other) === key;
    });
  }
  ...
  function sumCategoryActuals(plan, asOf, periodStart, opts) {
    ...
    for (const tx of packet.transactions) {
      ...
      if (skipPendingPostedDuplicate(tx, packet)) continue;
      const amt = Number(tx.amount);
      if (!isFinite(amt) || amt === 0) {
  ...
  function personalSpendOwner(tx) {
    ...
    if (!isAmazonPrimeMerchant(tx)
      && isAmandaAmazonShoppingAccount(accountText)
      && isAmazonMerchant(tx)) {
      return 'amanda';
    }
  ...
  function derivedTransactionFlags(tx) {
    ...
    if (!tx) return empty;
    const subscriptionBill = isAmazonPrimeMerchant(tx);
    return {
      ...
      personalOwner: subscriptionBill ? null : personalSpendOwner(tx),
      subscriptionBill,
      kind: subscriptionBill ? 'bill' : null,
    };
  }
