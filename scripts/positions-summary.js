...
// By institution, declared on the record — matching on the label missed the
...
/* --------------------------------------------------- the generated rows */
// [rowType, label] -> the full 21-column row.
...
put('Total revolving credit available', W('CREDIT', 'Total revolving credit available', 'Net', 'CAD',
  n2(util.totalAvailable), {
    note: 'Derived by Forecast.utilisation from the debt records plus the chequing overdraft. '
      ...
      + (util.unknownPendingCount
        ? `; ${util.unknownPendingCount} facility has unknown pending so that headroom is excluded`
        : '') }));
...
// The silver reached data.json and never reached here — that was the drift.
if (!sawSilver && SILVER) {
...
if (check) {
  ...
  console.log(`positions.csv regenerated — ${generated.size} computed rows${sawSilver ? '' : ' + silver row added'}.`);
}
