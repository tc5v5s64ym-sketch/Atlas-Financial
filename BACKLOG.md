...
`Forecast.baselineTrajectory` publishes walk-derived `debtDirection` from the incumbent coupled `projectDebts` month-end marks already used for monthly debt pictures and O1/O2 pressure. It answers which modelled debts decline, which remain persistent at whole-cent identity (not a comfort band), whether any supported balance increases, interest consequence the coupled walk established, and the first published month-end at which a modelled balance is $0 after a positive opening. Fail closed when fewer than two published debt marks exist; unpublished months contribute no invented direction. Planned Household Budget weeklyVariable does not manufacture future card borrowing. Available credit / headroom is not cash. Helper stays inside Forecast and is not exported. No ranking, recommendation, affordability, or owner-policy threshold. Planning reprint of debtDirection, packet/Talk reprint, Talk templates, Outcomes 4–5, scenarios, and `Forecast.recommend` remain out of scope.
