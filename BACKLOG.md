...
**B105o · Planning page — extra-debt-payment scenario on Road Ahead** · **IN PROGRESS** · *owner instruction; presentation only; Trajectory Dashboard Campaign Outcome 4*

`public/planning.js` / `planning.html` let the household supply an explicit eligible debt id and whole-cent additional payment amount, call `Forecast.baselineTrajectoryScenario` only, and reprint Baseline vs Scenario projected cash, named-debt walk marks, interest, and Forecast `delta` fields when published. Road Ahead baseline timeline and stage3 reprints stay on `Forecast.baselineTrajectory`; scenario does not mutate household evidence, infer amount, rank debts, or write. Unavailable fails closed. No `public/forecast.js` edits; Outcome 5, packet/Talk reprint, and other scenario classes remain out of scope.
