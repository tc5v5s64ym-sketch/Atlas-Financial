...
**B105a · Planning page reprints baselineTrajectory** · **DONE 2026-09-16** · *owner instruction; presentation only*

`public/planning.js` reprints `Forecast.baselineTrajectory` monthly income, month-end cash, and coupled debt marks — status, amounts, and unavailable notes only. Every horizon month prints, including 2027 months Forecast marks unavailable with notes. No simulate / expandEvents / recommend math on the page.

**B105b · Talk reprints baseline trajectory from planning.trajectory** · **IN PROGRESS 2026-09-16** · *owner instruction; Talk presentation only*

Allowlisted planning-horizon questions only: Gemini extract-only `planning-horizon-trajectory` intent, server reads `planning.trajectory` from the incumbent assistant packet, `talk-presentation.js` reprints every horizon month with Planning parity (including when `operatingPlan` is unavailable on forecast). Fail closed on payday / leftover phrasing collisions. No `decisionPosture`, no Forecast math, no packet/MCP changes.
